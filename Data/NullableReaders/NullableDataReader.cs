...
namespace Alvisoft.DataAccessLayer
{
    ...
    public sealed class NullableDataReader : IDataReader, INullableReader
    {
        ...
        public int GetIsNullInt32(int index, int valueNull)
        {
            int valueIsNull;
            if (reader.IsDBNull(index))
            {
                valueIsNull = valueNull;
            }
            else
            {
                valueIsNull = reader.GetInt32(index);
            }
            return valueIsNull;
        }

        public int GetIsNullInt32(string name, int valueNull)
        {
            return this.GetIsNullInt32(reader.GetOrdinal(name), valueNull);
        }

        public int GetIsNullInt32(string name)
        {
            return this.GetIsNullInt32(reader.GetOrdinal(name), 0);
        }
        ...
        public string GetIsNullString(string name)
        {
            return this.GetIsNullString(reader.GetOrdinal(name), "");
        }

        public string GetIsNullString(int index, string valueNull)
        {
            string valueIsNull;
            if (reader.IsDBNull(index))
            {
            ...
            {
                valueIsNull = reader.GetString(index);
            }
            return valueIsNull;
        }

        public string GetIsNullString(string name, string valueNull)
        {
            return this.GetIsNullString(reader.GetOrdinal(name), valueNull);
        }
